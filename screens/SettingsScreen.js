...
const SettingsItem = ({ title }) => {
  ...
  return (
    <TouchableOpacity style={styles.settingsItem}>
      <Text style={[styles.itemText, { color: isDarkTheme ? '#fff' : '#000' }]}>{title}</Text>
      <Ionicons name="chevron-forward" size={24} color={isDarkTheme ? '#fff' : 'black'} />
    </TouchableOpacity>
  );
};
  ...
  return (
    <View style={[styles.container, { backgroundColor: isDarkTheme ? '#101930' : '#fff' }]}>
      ...
      <View style={styles.themeSwitchContainer}>
        <Text style={[styles.themeText, { color: isDarkTheme ? '#fff' : '#000' }]}>Theme</Text>
        <Switch
          value={isDarkTheme}
